...
\title{\texttt{filterVcf()}:  Extract Variants of Interest from a Large VCF File}
\author{Paul Shannon}
...
Reading ``by chunks'' is accomplished using a tabix [\cite{li2011tabix}] file.
The \Rcode{yieldSize} argument specifies how many variant lines are read
...
repeatedly, with \Rcode{yieldSize} lines supplied on each invocation.
