.test <- function(dir, pattern = "^test_.*\\.R$")
{
    ...
    if (missing(dir)) {
        dir <- system.file("unitTests", package="VariantAnnotation")
        if (!length(dir)) {
            dir <- system.file("UnitTests", package="VariantAnnotation")
            if (!length(dir))
    ...
    suite <- defineTestSuite(name="VariantAnnotation RUnit Tests", dirs=dir,
    ...
    if (length(details <- .failure_details(result)) >0) {
        ...
        cat("\n\n")
        stop("unit tests failed for package VariantAnnotation")
    }
    result
}
