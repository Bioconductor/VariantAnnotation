...
From prior exploration we know that variants of biological interest
suitable for this demonstration can be found in a 1M base pair region of 
chromosome seven.  Extract just that region and write the variant calls
...
This code creates the small gzipped vcf and index files we use below.
...
hub <- AnnotationHub()
mcf7.gr <- 
    hub$goldenpath.hg19.encodeDCC.wgEncodeUwTfbs.wgEncodeUwTfbsMcf7CtcfStdPkRep1.narrowPeak_0.0.1.RData
@ 
\end{small}
...
ov.mcf7 <- findOverlaps(rowData(vcf), mcf7.gr)
@ 
\end{small}
