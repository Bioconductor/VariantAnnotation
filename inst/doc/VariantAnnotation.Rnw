...
Data can also be subset on the \Rcode{fixed}, \Rcode{info} and/or \Rcode{geno} fields 
in the VCF file. Possible fixed values are 'ALT', 'QUAL' and 'FILTER'. To discover 
available \Rcode{info} and \Rcode{geno} fields use \Rfunction{ScanVcfHeader} to 
inspect the header information. The return value is a list of length 3 with names of 
'Reference', 'Sample' and 'Header'. 
