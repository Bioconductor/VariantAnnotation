...
In this file we have only 1 ALT value per variant.
<<elementLengths_ALT>>=
...
accessor. These data have a unique value for each sample and 
the arrays can be multidimensional. 
Extract the genotype header information.
<<geno_hdr>>=
geno(hdr)
@
...
Elements of the genotype list are accessed in the usual way.
<<geno_subset>>=
geno(vcf)
geno(vcf)$GT[1:3,1:5]
@

In contrast to \Rcode{geno}, the \Rcode{info} data are unique
to the variant and the same across all samples. 
<<info>>=
info(vcf)[1:4, 1:5]
@
...
read in subsets of the data. Subsetting can be done by genomic 
coordinates (ranges) or by individual fields.

\subsubsection{Genomic coordinates}
To read in a portion of chromosome 22, we create a \Robject{GRanges} 
...
The names of the GRanges will be used later for subsetting the results.
<<names_subset_ranges>>=
names(rng) <- c("gene_79087", "gene_644186")
@

To subset by ranges, the VCF file must have a Tabix index file. For
help creating an index see ?\Rcode{indexTabix}. Once the index
exists, a \Rcode{TabixFile} is created. The \Rcode{TabixFile} is is a 
reference to the VCF that once opened will remain open across multiple
calls which avoids costly index re-loading. 

Create a \Rcode{TabixFile} for the sample data.
<<subset_TabixFile>>=
tab <- TabixFile(fl)
tab
@

Read the data in with \Rfunction{readVcf}. 
<<subset_call>>= 
vcf_rng <- readVcf(tab, "hg19", param=rng)
@

The 'dim' line in this output shows 397 records overlaped with the 
ranges specified in the param. 
<<subset_call>>= 
vcf_rng
@
...
In addition to subsetting by ranges, specific fields in the VCF
file can be requested. View the header information before reading 
in the data to determine what variables are available for import.
...
for linkage disequlibrium', and "GT" in FORMAT which is 'genotype'.
<<subset_elements>>=
...
Unlike subsetting by ranges, subsetting by VCF fields does not require
a \Robject{TabixFile}. 
...
The subsetting involves genomic coordinates so we need to use the 
\Robject{TabixFile}.
<<subset_both>>=
...
\Rfunction{locateVariants} outputs a \Robject{GRanges} with one row
for each variant-transcript match. The \Rcode{metadata} columns returned 
are \Rcode{LOCATION}, \Rcode{QUERYID}, \Rcode{TXID}, \Rcode{CDSID}, 
\Rcode{GENEID}, \Rcode{PRECEDEID} and \Rcode{FOLLOWID}. The last two
columns only have values in the case of \Robject{IntergenicVariants}.
\Rcode{FOLLOWID} are also included. The \Rcode{QUERYID} column maps back 
to the row number in the original \Robject{query}. 

To answer gene-centric questions data can be summarized by gene reguardless
of transcript. 

Did any coding variants matched more than one gene?
