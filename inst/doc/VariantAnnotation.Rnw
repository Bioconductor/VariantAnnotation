...
As sample data we use the \Robject{GRanges} object, \Rcode{variants},
that contains both real and faux variants. The ranges in the \Rcode{variants} 
...
The reference and variant alleles are contained in the \Rcode{refAllele} and 
\Rcode{varAllele} columns as \Robject{DNAStringSet}s. An insertion 
...
The \Rfunction{locateVariants} function returns a \Robject{DataFrame} the
same length as the \Robject{query} argument with columns for the location and
transcript and gene ID. For intergenic variants, gene ID's for the 
genes on either side of the variant are provided.
...
If a variant matches multiple regions the highest priority position is 
chosen. For example, if there is a match to both an intron and 5' UTR region,
the data for the 5' UTR region will be returned. 
...
\hline
coding & 1 & variant falls \emph{within} a coding region \\
...
%definition \\
UTR5 & 2 & variant overlaps \emph{any} portion of a 5' untranslated region \\
UTR3 & 2 & variant overlaps \emph{any} portion of a 3' untranslated region \\
intron & 3 & variant overlaps \emph{any} portion of a transcript but does not
...
%site \\
intergenic & 4 & variant does not overlap \emph{any} portion of a transcript \\
\hline
\end{tabular}
\end{center}
...
between the variants in the \Rcode{query}, a 
\Rcode{GRanges} object, and the coding regions of a \Robject{TranscriptDb} 
object. Reference sequences for the codons are retrieved from either a 
\Robject{BSgenome} object or fasta file.
Codons sequences for the variants are produced by substituting,
inserting or deleting the alleles specified in the varAllele column
into the reference sequences. When the resulting codon sequence is a multiple 
of 3, amino acid codes are computed for the variant codon sequence. Table 
\ref{table:aacoding} contains examples of situtions when amino acid codes 
are and are not computed for the variant codon sequence. The insertion position 
of the variant alleles in Table \ref{table:aacoding} are random and were chosen 
...
\hline
refAllele & varAllele & refCodon & varCodon & AA coding provided & Rationale \\
\hline
G & T & agg & agT & yes & snp result is multiple of 3\\
...
GGCCTA & `' & acggcctaa & aca & yes & deletion results is multiple of 3\\
\hline
\end{tabular}
\end{center}
...
The coding variants fall in chromosomes 1, 2 and 16. Here we create a 
\Robject{GRanges} of coding regions by transcripts and retian only the 
chromosomes of interest to use as the \Rcode{subject}. Alternatively we 
could use the full \Robject{TranscriptDb}.
...
tx\_id, refCodon, varCodon, refAA, varAA, Consequence and any
...
\Rcode{variants} object fell within a coding region and matched four transcripts. 
...
5' utr, etc.) and for presence of the variant in dbSNP.
...
regfilt <- regionFilter(txdb, region="coding")
regfilt(variants)

# both filters as a single filter
combofilt <- compose(snpfilt, regfilt)
combofilt(variants)
