...
This document illustrates the annotation of genetic variants using the 
\VariantAnnotation package and existing Bioconductor tools. 

Predicting the effects that genetic variants have on known 
transcripts is valuable for priorization of variants. This information
may aid in the selection of variants in genotyping studies and play a role
in the discovery of new drugs and other biologically significant
loci.

A variety of tools exist for the annotation of genetic variants ranging
from web-based interfaces where data are uploaded to command line
software that can be downloaded and run locally. ANNOVAR,
SeattleSeqAnnotation, and Ensembl's Variant Effect predictor
are a few recent contributions. In general, these tools
provide information regarding the functional consequence on genes
and provide a method of filtering the variants based on some
user-specified criteria. 


%A brief outline of the capability of each tool
%follows.
%
%\subsection{ANNOVAR}
...
The insertion or deletion of nucleotides that are not multiples of 3 result
are categorized as a frameshift.
...
basepairs \\
-/G & no & frameshift & insertion with incompatible number of replacement
basepairs \\
-/TTG & yes & synonymous or nonsynonymous & insertion with compatible number of replacement
basepairs \\
A/- & no & frameshift & deletion with incompatible number of replacement
basepairs \\
GGCCTA/- & yes & synonymous or nonsynonymous & deletion with compatible number of replacement
basepairs \\
\hline
\end{tabular}
\end{center}
