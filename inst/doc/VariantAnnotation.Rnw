...
sustituted into the \Rcode{refCodon} \Rcode{CAC} to produce \Rcode{varCodon} 
\Rcode{CAA}. The \Rcode{refCodon} is the sequence of codons necessary to make 
the variant allele substitution and therefore often includes more nucleotides 
than present in the range (i.e., 50301494, 50301494). Notice that it is the 
third position in the \Rcode{refCodon} that has been substituted. This third 
position in the codon, the position of substitution, corresponds to genomic 
position 50301494. This genomic position maps to position 867 in coding 
region-based coordinates and to triplet 289 in the protein. This is a 
non-synonymous coding variant where the amino acid has changed from 
\Rcode{H} (His) to \Rcode{Q} (Gln).
