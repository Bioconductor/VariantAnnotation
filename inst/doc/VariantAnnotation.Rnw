...
retrieved with the \Rfunction{exptData} accessor. To view all of REF, ALT, QUAL, 
and FILTER together use the \Rfunction{fixed} accessor.
<<readVCF_info>>=
## View the header information related to the INFO fields
head(exptData(vcf)[["HEADER"]][["INFO"]], 3)

## Access all fixed fields with 'fixed'
fixed(vcf)[1:3,] 
## or individual fields with their own accessors.
ref(vcf)[1:3,] 
qual(vcf)[1:3,] 
...
\Robject{param}. The resulting \Robject{VCF} object shows that 27 of the total 
1000 variants overlaped with our defined ranges and were read in. 
<<subset_call>>= 
...
To discover available \Rcode{info} and \Rcode{geno} fields read in the header
information with \Rfunction{ScanVcfHeader}. The return value is \Rclass{vcfHeader} 
instance. Accessors for this class are the same as for the \Rclass{VCF} class,
\Rcode{fixed}, \Rcode{info} and \Rcode{geno}. Other accessors and more
...
Fields in the fixed, info or geno fields of a VCF file often have more than 
one value per record. The \Robject{VCF} object created by \Rfunction{readVcf} 
stores these fields as compressed lists or arrays. It may be useful to have 
an expanded form of the data where each row represents a record with a unique 
combination of elementMetadata values. The elementMetadata columns are 
the fixed, info or geno fields specified in the ScanVcfParam object.

To create a long form GRanges, we set \Rcode{asGRanges=TRUE} in the 
\Robject{ScanVcfParam} object and specify \Rcode{fixed}, \Rcode{info} and/or 
\Robject{geno} elements. Possible values for \Rcode{fixed} are ALT, QUAL or 
FILTER. 

\Rcode{info} and \Rcode{geno} information was previously collected from the 
file header. We revisit these variables to see the fields available for 
import.
<<longform_header>>=
rownames(info_DF)
rownames(geno_DF)
...
In the \Robject{ScanVcfParam} we specify \Rcode{asGRanges=TRUE} and the desired 
fields. We will request one \Rcode{info} field and one \Rcode{fixed}. Keep in 
mind the data are expanded to accomodate all combinations of the requested 
fields and can get big very quickly. 
<<expand_info>>=
param <- ScanVcfParam(asGRanges=TRUE, fixed="ALT", info="HOMSEQ")
lf <- readVcf(vcffile, "hg19", param)
head(lf, 4)
@
