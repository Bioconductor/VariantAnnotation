...
The alternate allele (ALT) in the VCF file often has more than one 
value per record. A long form \Robject{GRanges} is one where the 
rows are replicated to match the length of the `unlisted' alternate 
allele column. This format provides a row for each REF and ALT 
combination for all variants.

The \Rcode{readVcfLongForm} function reads data from a VCF file in the 
same manner as \Rcode{readVcf}. Input arguments and the ability to subset 
data on ranges or VCF fields are the same. Data from the \Rcode{fixed} and 
\Rcode{info} fields are also expanded as elementMetadata columns. Currently 
no \Rcode{geno} information is included.

\Rcode{info} and \Rcode{geno} information was previously collected from the 
file header. We revisit these variables to select fields available for 
import.
<<longform_header>>=
rownames(info_DF)
rownames(geno_DF)
...
<<expand_info>>=
param <- ScanVcfParam(asGRanges=TRUE, fixed="ALT", info="HOMSEQ")
