...
a 'long form' \Robject{GRanges} which is an expanded form of the VCF data.
...
\Rcode{exptData} slot. Extract the header information
<<readVCF_showheader>>=
...
Values in the \Rcode{paramRangeID} column are meaningful when ranges are
specified in the the \Rcode{param} argument to \Rfunction{readVcf}.
This is discussed further in the `Reading Subsets of Data' section.
...
Data from the \Rcode{INFO} field can be accessed with the \Rcode{info} 
accessor. 
<<readVCF_info>>=
## Header information related to INFO fields
head(info(hdr), 3)
...
The \Rfunction{fixed} accessor will retrieve all of REF, ALT, QUAL, and FILTER.
<<>>=
fixed(vcf)[1:3,] 
@

These values can be accessed individually with their own accessors.
<<>>=
qual(vcf)[1:3,] 
filt(vcf)[1:3,] 
@

The genotype data described in the \Rcode{FORMAT} field are parsed into 
...
because the genotype data are unique for each sample and the data structures 
can be multidimentional. In contrast, the \Rcode{fixed} and \Rcode{info} data 
are the same across all samples.

Header information for the genotype data is in \Rcode{exptData} just as the
\Rcode{info} data was.
<<>>=
geno(hdr)
@
...
rng <- GRanges(seqnames="22", 
               ranges=IRanges(c(51151293, 51151684), c(51151670, 51152265)))
names(rng) <- c("geneA", "geneB")
@
...
\Robject{param}. The dimention of the resulting \Robject{VCF} object shows 
19 records overlaped with the specified ranges. 
<<subset_call>>= 
...
Data can also be subset on the \Rcode{fixed}, \Rcode{info} and/or \Rcode{geno} 
fields in the VCF file. Possible fixed values are 'ALT', 'QUAL' and 'FILTER'. 
Discovery of available \Rcode{info} and \Rcode{geno} fields can be accomplished
by reading header information with \Rfunction{ScanVcfHeader}. The return value 
is  a \Rclass{VCFHeader} instance. Accessors for this class are the same as for 
the \Rclass{VCF} class, \Rcode{fixed}, \Rcode{info} and \Rcode{geno}. Additional 
information can be found at ?\Rcode{VCFHeader}.
...
vcf_flds <- readVcf(vcffile, "hg19", svp)
geno(vcf_flds)
...
\Rfunction{locateVariants} matches the ranges in the \Rcode{query}
to regions in the \Robject{subject} annotation object. Metadata columns may be 
present in the \Robject{query} but they are ignored. When the \Rcode{query} is
a \Robject{VCF} the variant ranges come from the \Rcode{rowData} slot.

Specific regions are queried by specifying the \Rcode{region} argument as one 
of CodingVariants, IntronVariants, FiveUTRVariants, ThreeUTRVariants, 
IntergenicVariants, SpliceSiteVariants or AllVariants.

The \Rcode{CodingVariants()} constructor creates an instance of a
\Robject{CodingVariants} class and is used directly in the function call.
<<CodingVariants>>=
loc <- locateVariants(rd, txdb, CodingVariants())
head(loc, 4)
@

Variants overlapping the first 2 or last 2 nucleotides of an intron are
considered to fall in splice sites. They are specified with 
\Rcode{SpliceSiteVariants}.
<<SpliceSiteVariants>>=
head(locateVariants(rd, txdb, SpliceSiteVariants()), 4)
@

To locate variants in all regions,
<<AllVariants, eval=FALSE>>=
locateVariants(rd, txdb, AllVariants())
@

\Rfunction{locateVariants} returns a \Robject{GRanges} with columns of 
\Rcode{locate}, \Rcode{queryID}, \Rcode{txID}, \Rcode{cdsID}, and 
\Rcode{geneID}. In the case of \Robject{IntergenicVariants} columns for
\Rcode{precedesID} and \Rcode{followsID} will also be included. The
\Rcode{queryID} column maps back to the row number in the original 
\Robject{query} input. Each row in the \Rfunction{locateVariants} result
represents a variant-transcript match and therfore multiple rows per variant 
are possible. Values in the \Rcode{location} column are one of the variant 
regions as described in Table \ref{table:location}. 
...
intergenic & does not fall \emph{within} a transcript \\
...
\end{tabular}
\end{center}
\caption{Variant Regions}
\label{table:location}
\end{table}

Each row of the results represents a variant-transcript match. To answer 
gene-centric questions data can be summarized by gene reguardless of transcript.

Did any variants match more than one gene?
<<locatVariants_example1>>=
table(sapply(split(values(loc)[["geneID"]], values(loc)[["queryID"]]), 
    function(x) length(unique(x)) > 1))
@

Summarize the number of variants by gene ID
<<locatVariants_example2>>=
...
The result is a modified \Rcode{query} containing only variants that fell in 
coding regions. Each row represents a variant-transcript match so more than
one row per original variant is possible.

<<predictCoding>>=
library(BSgenome.Hsapiens.UCSC.hg19)
aacoding <- predictCoding(vcf, txdb, seqSource=Hsapiens)
head(aacoding, 4)
@

If translation was possible, the consequence column will be `synonymous' or 
`nonsynonymous'. Where translation was not possible, consequence will be 
`frameshift' and the \Robject{varAA} value will be missing. 

<<predictCoding_frameshift>>=
## consequence is 'frameshift' where translation is not possible
head(aacoding[values(aacoding)[["consequence"]] == "frameshift"], 3)
@
...
<<nonsynonymous>>=
nonsyn <- aacoding[values(aacoding)[["consequence"]] == "nonsynonymous"]
rsids <- names(nonsyn)[grep("rs", names(nonsyn), fixed=TRUE)]
rsids
@
...
gr <- readVcfLongForm(vcffile, "hg19", param)
head(gr, 4)
@
