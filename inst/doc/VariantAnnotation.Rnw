...
variant is non-synonymous and the amino acid codes for the change. If a
...
%The table below outlines how variant location is determined.
%The row names are the classification and column names are the region
%that was matched by the variant.
%\begin{table}[ht]
%\begin{center}
%\begin{tabular}{l|l|l|l|l|l}
%\hline
%Region & Transcript & Coding Region & 5UTR & 3UTR \\
%\hline
%exon & yes &  yes & NA & NA \\
%intron & yes & no & NA & NA \\
%5UTR & yes & no & yes & no \\
...
%definition \\
UTR5 & 2 & variant overlaps a 5' untranslated region \\
UTR3 & 2 & variant overlaps a 3' untranslated region \\
intron & 3 & variant overlaps an intron \\
...
exonic variants we are intrested in are chromosomes 1, 2 and 16. We create a
...
matched an exon region. Three new columns of metadata,
txID, refAA, and obsAA contain the transcript ID's
and reference and observed amino acid codes. 

<<coding_result>>= 
aaCoding <- predictCoding(variants, grl, variantAllele="obsAllele",
    seqSource=Hsapiens)
head(aaCoding[,c("txID", "refAA", "obsAA")])
aaCoding[1:4,]
@
Each row of the reult represents a transcript
matched by a variant. If a variant matched multiple
transcripts, we see multiple rows for that variant.
For example, we see the variant in the sixth row of the 'variants'
data fell within an exon and hit four transcripts. The 'aaCoding'
result shows four rows for this variant, one for each transcript.
