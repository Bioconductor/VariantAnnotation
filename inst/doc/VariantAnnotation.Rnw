...
The \Rfunction{readVcf} function parses data from a VCF file into a 
\Rfunction{VCF} object. \Rfunction{locateVariants} categorizes variants with 
respect to gene function, such as 'intron', 'coding', 'UTR' or 'intergenic' 
region. Amino acid coding changes for the non-synonymous variants can be 
computed with \Rfunction{predictCoding}. SIFT and PolyPhen databases can be 
queried to provide predictions of how severly these amino acid coding changes 
may affect protein function. Filtering functions allow the subsetting of 
variants on physical location or membership in dbSNP. 
...
Data are parsed into a \Robject{VCF} object with \Rfunction{readVcf}. Header 
information from the VCF file is stored as a \Robject{SimpleList} in the 
...
of the data. Data can be subset by selecting specific fields from the VCF or by
selecting parts of the genome or both.

\subsubsection{Subset on genomic coordinates}
...
help creating an index. An index file for our sample file is included with the 
package. 

Once an index exists we create \Robject{TabixFile} instance. See 
...
\Robject{param}. The resulting \Robject{VCF} object shows 27 of the total 1000
variants overlaps with our defined ranges and were read in. 
<<subset_call>>= 
vcf_rng <- readVcf(tab, "hg19", param=rng)
vcf_rng
@

\subsubsection{Subset on VCF fields}
...
\subsection{Create a flat GRanges}
