...
A subset of data can be read in by specifying ranges, geno and info fields in
a \Robject{ScanVcfParam}. If subsetting consists only of ranges, a \Rcode{GRanges} 
can be used as the \Rcode{param} argument. To access a subset of ranges the VCF 
file must have an accompanying Tabix index file. See ?\Rcode{indexTabix} for 
help creating an index file.
...
The CHROM, POS, ID, REF, ALT, QUAL and FILTER fields of the VCF file are parsed 
...
\Rcode{assays} slot to a \Robject{SnpMatrix}. The return value is a named list 
...
<<snpMatrix>>=
## Genotypes have been parsed into the "GT" matrix in the assays slot.
calls <- assays(vcf)$GT
mapgr <- rowData(vcf)

MatrixToSnpMatrix(calls, mapgr)
@
