...
<<subset_call>>= 
vcf_rng <- readVcf(tab, rng, "hg19")
vcf_rng
@

\subsubsection{VCF fields}
Data can also be subset on the \Rcode{INFO} and \Rcode{FORMAT} fields in the 
VCF file. To discover what \Rcode{INFO} and \Rcode{geno} fields are available 
for subsetting we use \Rfunction{ScanVcfHeader} to inspect the header 
information. The return value is a list of length 3 with names of 'Reference', 
'Sample' and 'Header'. 
...
vcf_flds <- readVcf(vcffile, svp, "hg19")
vcf_flds
@
...
\subsection{Long form GRanges - under construction}
...
lists or arrays. It may be useful to have an expanded form of the data where 
each row represents a variant with a single value. This can be accomplished by
setting an option in the \Robject{ScanVcfParam} object.

Specify \Rcode{asGRanges=TRUE} and the \Rcode{info} or \Rcode{geno} fields to
be expanded. Here we specify two fields of \Rcode{info}, 
<<expand_info, eval=FALSE>>=
param <- ScanVcfParam(asGRanges=TRUE, info=c("LDAF", "HOMSEQ"))
#iflat <- readVcf(vcffile, param, "hg19")
#head(iflat)
@
...
#gflat <- readVcf(vcffile, param, "hg19")
@
