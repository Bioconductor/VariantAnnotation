...
Amino acid coding changes for variants that fall 
within coding regions can be determined with \Rfunction{predictCoding}. 
Overlaps are identified between a list of variants
and the coding regions of a \Robject{TranscriptDb} object. 
Sequences for the variants that fall in coding regions are 
retrieved from either a \Robject{BSgenome} object or fasta file and
the change in amino acid coding is computed using the the variant 
alleles provided in the \Robject{query}. Table \ref{table:aacoding} 
...
\hline
alleles & AA Coding & Consequence & Details \\
\hline
...
The coding variants we are intrested in fall in chromosomes 1, 2 and 16. 
...
Our result is a \Robject{GRanges} containing only the variants that
matched a coding region. New metadata columns
...
aaCoding <- predictCoding(variants, grl, alleles="alleles",
    seqSource=Hsapiens)
head(values(aaCoding)[c("alleles", "CodonChange", "AAChange", "Consequence")], 6)
@
