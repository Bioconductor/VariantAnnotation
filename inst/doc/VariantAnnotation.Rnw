...
a subset of ranges to read in. \Rcode{VariantAnnotation} does not
have a sample compressed VCF file so we create one on the fly,
then create a tabix index and retrieve a subset of ranges.
...
are methods for predicting the impact of amino acid substitution on a
human protein. The SIFT method uses sequence homology and the physical properties 
of amino acids to make predictions about protein function. PolyPhen uses 
sequence-based features and structural information characterizing the substitution 
...
subst <- c("rsid", "prediction", "score", "aa_change", "protein_id")
select(SIFT.Hsapiens.dbSNP132, keys=rsids, cols=subst)

select(PolyPhen.Hsapiens.dbSNP131, keys="rs3026284", 
    cols=c("training_set", "prediction", "pph2_prob", "pph2_FPR", "pph2_FDR"))
@

Due to the large number of columns in the PolyPhen database, detailed
descriptions were included on a separate man page. See ?PolyPhenDbColumns.
